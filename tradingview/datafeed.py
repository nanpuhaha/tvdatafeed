...
class TvDatafeed:
    ...
    def __create_df(raw_data, symbol):
        data = []
        for match in re.findall(r'"s":(\[.+?\}\])', raw_data):
            data.extend([dic['v'] for dic in eval(match)])
    ...
    def get_hist(
        ...
        Args:
            ...
            n_bars (int, optional): no of bars to download, max 5000. Defaults to 5000.
        ...
        while True:
            ...
            if "series_completed" in result:
                count += 1
                if show_process:
                    print(f"request more data {count}")
                if count >= request_more_data_count:
                    break

                self.__send_message(
                    "request_more_data",
                    [self.chart_session, "s1", n_bars],
                )

        return self.__create_df(self.raw_data, symbol)
